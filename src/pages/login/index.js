import React, { Component } from 'react';
import SweetAlert from 'sweetalert2-react';
import UserDetails from '../../userDetails';
import './index.scss';

class Login extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showAlert: false,
      alertText: null,
      passwordError: null,
      usernameError: null,
      passwordValue: null,
      usernamevalue: null
    };
  }

  handleChange = evt => {
    this.formValidaton(evt);
  }

  formValidaton = evt => {
    const name = evt.target.name;
    const value = evt.target.value;
    let msg = ``;
    switch (name) {
      case 'username': {
        msg = value === '' ? `Username should not be empty*` : null;
        this.setState({ usernameError: msg, usernamevalue: value });
        break;
      }
      case 'password': {
        msg = value === '' ? `Password should not be empty*` : null;
        this.setState({ passwordError: msg, passwordValue: value });
        break;
      }
      default:
    }
  }

  isFormVald = () => {
    if ((this.state.usernameError === null && this.state.passwordError === null) 
        && (this.state.usernamevalue !== null && this.state.passwordValue !== null)) {
      this.login()
    } else {
      this.setState({showAlert: true, alertText: 'Please enter your username and password'})
    }
  }

  login = () => {
    const isloginValid = this.checkUser();
    if (isloginValid){
      localStorage.setItem('username', this.state.usernamevalue)
      this.props.history.push('/task-dashboard');
    } else{
      this.setState({ showAlert: true, alertText: 'Invalid credential' })
    }
  }

  checkUser = () => {
      const loginResponse = UserDetails.filter((element, i) => {
        if (element.username === this.state.usernamevalue && element.password === this.state.passwordValue) {
          return element;
        }
        return null;
      })
      return loginResponse.length > 0;
  }

  render() {
    return (
      <div className="login__page-container">
        <SweetAlert
          show={this.state.showAlert}
          title=""
          text={this.state.alertText}
          onConfirm={() => this.setState({ showAlert: false })}
        />
        <div className="login-form">
          <div className="imgcontainer">
            <img src="data:image/jpeg;base64," alt="Avatar" className="avatar" />
          </div>
          <div className="container">
            <label for="uname"><b>Username</b></label>
            <input type="text" placeholder="Enter Username" name="username" onChange={(e) => this.handleChange(e)} />
            <p className="error-msg">{this.state.usernameError}</p>
            <label for="psw"><b>Password</b></label>
            <input type="password" placeholder="Enter Password" name="password" onChange={(e) => this.handleChange(e)} />
            <p className="error-msg">{this.state.passwordError}</p>
            <button type="submit" onClick={() => this.isFormVald()}>Login</button>
          </div>
        </div>
        <div className="credential-data">
          <span>Login with these credention</span>
          <pre>username: 'jhon',<br></br>
                password: 'Qwerty@123'<br></br>
                username: 'clark',<br></br>
                password: 'Qwerty@123'<br></br>
                username: 'bruce',<br></br>
                password: 'Qwerty@123'<br></br>
          </pre>
        </div>
      </div>
    );
  }
}

export default Login;
